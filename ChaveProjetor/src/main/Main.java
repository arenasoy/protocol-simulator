...
public class Main {
	...
	public static void main(String[] args) {
		...
		do {
			p = c.readInt("Em qual porta o servidor da chave do projetor deve iniciar? (recomendado = 9001-9999)");
		...
		do {
			...
			if(s == null || !m.matches()) {
				System.out.println("Endereco invalido! Deve ser no formato endereco:porta. Exemplos: dominio.com:9000 ou 192.168.0.110:8500.");
			}
		} while(s == null || !m.matches());

		try {
			chave = new Chave(m.group(1).trim(), Integer.parseInt(m.group(2)), p);
			System.out.println("O servidor esta hospedado em: " + chave.getHostAddress() + ":" + chave.getPort());
			chave.execute();
		} catch(Exception ex) {
